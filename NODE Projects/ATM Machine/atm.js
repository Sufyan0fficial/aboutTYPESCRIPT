...
// }
// else
// console.log(errorclr('InValid Input / Please Re-enter your PIN '));
// //ATM Functionalities 
...
//         console.log(`Dear Customer your Available Balance is : ${highlight(user.balance)}`)
//     }
//     if (userAction.select === 'Withdraw Money') {
//         let Withdraw = await inquirer.prompt({
//             type : 'number',
//             message : `Please Enter the amount in ${highlight('multiples of 500')} : `,
//             name : 'amount',
//             validate :((input: number) => (input > 0 && input <= user.balance) || errorclr("Insufficient Balance ")) && (input => (input % 500 === 0) || `Invalid amount entered...\n${errorclr('Please enter an amount that is a multiple of 500')}`) 
//             }
//         )
//         if(true) {
//             let confirm = await inquirer.prompt({
//                 type :'confirm',
//                 message : `${chalk.italic(`\nPlease confirm the details before proceeding`)} \n${confirmDetails(`Account Hodler : ${user.Name}\nAccount No. : ${user.account}\nAmout to Withdraw : ${Withdraw.amount}`)}
//                 \nEnter ${highlight('Y')} to Proceed : `,
//                 name : 'details',
//                 })
//             if (confirm.details) console.log(`${chalk.green(`Transaction Successful !`)}\nRemainig Balance : ${highlight(user.balance - Withdraw.amount)}`);
//             else
//             console.log(`${errorclr('Transaction Failed !')}`);
//         }
//     }
